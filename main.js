...
*/
    ...
    for(let name in Game.rooms) {
        if (Game.rooms[name].controller && Game.rooms[name].controller.my) {
            ...
            let totalButlers = _.sum(Game.creeps, (c) => c.memory.role === 'butler'); // Alt
            let upgraders = _.filter(Game.creeps, (creep) => creep.memory.role === 'upgrader');
            let builders = _.filter(Game.creeps, (creep) => creep.memory.role === 'builder');
            let totalJanitors = _.sum(Game.creeps, (c) => c.memory.role === 'janitor'); // Alt
            let totalDefenders = _.sum(Game.creeps, (c) => c.memory.role === 'defender'); // Alt
            ...
            if (totalHarvesters < 5 && energyFull) {
                newName = 'Har ' + Game.time;
                Game.spawns['Spawn1'].spawnCreep( spawnBody(Math.min(energyAvailable, 500), ['WORK', 'WORK'], ['CARRY', 'CARRY', 'MOVE', 'MOVE', 'CARRY', 'CARRY']),
                    newName,
                    { memory: { role: 'harvester', targetId: null, sourceId: null }});
            } else if (totalButlers < 2 && energyFull) {
                newName = 'But ' + Game.time;
                Game.spawns['Spawn1'].spawnCreep(spawnBody(Math.min(energyAvailable, 400), ['CARRY', 'CARRY'], ['MOVE', 'MOVE', 'CARRY']),
                    newName,
                    {memory: {role: 'butler', targetId: null, sourceId: null}});
            } else if (upgraders.length < 1 && energyFull) {
                newName = 'Upg ' + Game.time;
                Game.spawns['Spawn1'].spawnCreep( spawnBody(Math.min(energyAvailable, 450), ['WORK', 'WORK'], ['CARRY', 'MOVE', 'CARRY', 'CARRY']),
                    newName,
                    { memory: { role: 'upgrader', sourceId: null }});
            } else if (builders.length < 1 && energyFull) {
                newName = 'Bld ' + Game.time;
                Game.spawns['Spawn1'].spawnCreep( spawnBody(Math.min(energyAvailable, 350), ['WORK', 'WORK'], ['CARRY', 'MOVE', 'CARRY']),
                    newName,
                    { memory: { role: 'builder', targetId: null, sourceId: null }});
            } else if (totalJanitors < 1 && energyFull) {
                newName = 'Jan ' + Game.time;
                Game.spawns['Spawn1'].spawnCreep(spawnBody(Math.min(energyAvailable, 500), ['WORK', 'WORK'], ['CARRY', 'MOVE', 'MOVE', 'CARRY']),
                    newName,
                    {memory: {role: 'janitor', targetId: null, sourceId: null}});
            } else if (totalDefenders < 1 && energyFull) {
                newName = 'Def ' + Game.time;
                Game.spawns['Spawn1'].spawnCreep( spawnBody(Math.min(energyAvailable, 300), ['RANGED_ATTACK'], ['MOVE', 'RANGED_ATTACK'], ['TOUGH']),
                    newName,
                    { memory: { role: 'defender', hostileId: null }});
            }
            if (totalHarvesters === 0 /*&& !energyFull*/) { // try to buy cheap harvester
                ...
                newName = '!But ' + Game.time;
                Game.spawns['Spawn1'].spawnCreep([CARRY, MOVE],
                    newName,
                    { memory: { role: 'butler', targetId: null, sourceId: null }});
            }
            /*else {
